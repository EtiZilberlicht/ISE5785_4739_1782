...
public class Camera implements Cloneable {
    ...
    *                   {@code oldCamera}
    */
   public static Builder getBuilder(Camera oldCamera) { return new Builder(oldCamera); }
   ...
   public static class Builder {
       ...
       */
      public Builder(Camera oldCamera) { this.camera = oldCamera; }
      ...
      private static final String DESCRIPTION      = "Missing data";
      private static final String POSITIVE_MESSAGE = "must be positive";
       ...
       */
      public Camera build() {
         checkAndUpdateCameraVectors();

         checkResolutionAndCreateImageWriter();

         checkAndGenerateViewPlaneData();

         if (!isZero(rotationAngleDegrees)) rotateCamera();
         if (transition != null) translateCamera();
      ...
      private void generateRayTracer() { //
         camera.rayTracer = switch (tracerType) {
         case GRID -> new GridRayTracer(scene);
         default -> new SimpleRayTracer(scene);
         };
      }

      private void checkAndUpdateCameraVectors() {
         if (camera.cameraPoint == null)
            throw new MissingResourceException(DESCRIPTION, CLASS_NAME, "cameraPoint");
         if (camera.vUp == null)
            throw new MissingResourceException(DESCRIPTION, CLASS_NAME, "vUp");
         if (camera.vTo == null)
            throw new MissingResourceException(DESCRIPTION, CLASS_NAME, "vTo");
      ...
      private void checkAndGenerateViewPlaneData() {
         if (alignZero(camera.distance) <= 0)
            throw new IllegalArgumentException("distance " + POSITIVE_MESSAGE);
         camera.viewPlanePC = camera.cameraPoint.add(camera.vTo.scale(camera.distance));
         if (alignZero(camera.width) <= 0)
            throw new IllegalArgumentException("width " + POSITIVE_MESSAGE);
         if (alignZero(camera.height) <= 0)
            throw new IllegalArgumentException("height " + POSITIVE_MESSAGE);
         camera.rY = camera.height / camera.nY;
         camera.rX = camera.width / camera.nX;
      }

      private void checkResolutionAndCreateImageWriter() {
         if (alignZero(camera.nX) <= 0)
            throw new IllegalArgumentException("nX " + POSITIVE_MESSAGE);
         if (alignZero(camera.nY) <= 0)
            throw new IllegalArgumentException("nY " + POSITIVE_MESSAGE);
         camera.imageWriter = new ImageWriter(camera.nX, camera.nY);
      }

      private void translateCamera() {
         camera.cameraPoint = camera.cameraPoint.add(transition);
      ...
      private void rotateCamera() {
         double angleDeg = rotationAngleDegrees % 360;
         if (angleDeg < 0)
            angleDeg += 360;

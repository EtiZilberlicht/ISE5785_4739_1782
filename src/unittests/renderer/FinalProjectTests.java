...
public class FinalProjectTests {
   ...
   public FinalProjectTests() { /* it is empty */ }

   /** The setting for multi-threading test runs */
   private static final int     THREADS       = -2;
   /** The scene for the test */
   private static final Scene   scene         = minecraft(0, 1);
   // private static final Scene scene = minecraft(9, 49); // with soft shadows
   ...
   private Testable[]           tests         =
      {
        new Testable(true, () -> testFinalProject(0, RayTracerType.SIMPLE)),
        new Testable(true, () -> testFinalProject(THREADS, RayTracerType.SIMPLE)),
        new Testable(true, () -> testFinalProject(0, RayTracerType.GRID)),
        new Testable(true, () -> testFinalProject(THREADS, RayTracerType.GRID))
      };

   @Test
   public void testMinecraft() { //
      cameraBuilder.build();
      cameraBuilder.setTransition(null).setRotation(0);
      for (var t : tests) if (t.run) t.test.run();
   }
   ...
   private void testFinalProject(int multithreading, RayTracerType type) {
      ...
      camera.renderImage();
      System.out.printf("    Measurement - %.2fs%n", (System.nanoTime() - start) / 1E9);
   ...
   private static Scene minecraft(double lightSize, int softShadowsSamples) {
      ...
      scene.geometries.add(/* new Sphere(10, new Point(0, 15, 20)), */
                           ...
                           new Sphere(25, new Point(0, 40, -160)).setEmission(new Color(30, 20, 5))
                              .setMaterial(new Material().setKD(0.05).setKS(0.95).setShininess(300).setKT(0.9)
                                 .setKR(0.1)),
                           ...
                           new Plane(new Point(-25, -27, 60), Vector.AXIS_Y).setEmission(new Color(181, 101, 29))
                              .setMaterial(
                                           skyM),
                           new Plane(new Point(0, 40, -800), Vector.AXIS_Z).setEmission(skyC).setMaterial(skyM));

      scene.lights.addAll(List.of(
                                  new PointLight(new Color(YELLOW), new Point(0, 40, -160)).setKL(0.0001)
                                     .setKQ(0.00001),
                                  new SpotLight(new Color(1000, 800, 600), new Point(-200, 20, -100),
                                                new Vector(2, -1, 1))
                                     .setKL(0.00002)
                                     .setKQ(0.00003).setShape("cycle").setSize(lightSize)
                                  ...
                                  new SpotLight(new Color(500, 500, 600), new Point(-10, 20, 80),
                                                new Vector(16, -18, -57))
                                     .setKL(0.00002)
   ...
   /** Record to define a conditional test */
   private static record Testable(boolean run, Runnable test) {
   }
}
